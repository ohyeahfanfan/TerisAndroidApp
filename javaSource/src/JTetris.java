...
public class JTetris extends JComponent implements Runnable {
	...
	protected static pieceQueue sharedQueue;
	private ObjectInputStream serIn;
	private ObjectOutputStream serOut;
	 ...
	 */
	JTetris(int pixels, ObjectInputStream in, ObjectOutputStream out, int[] sharedInt) {
		super();
		...
		this.sharedQueue = new pieceQueue(16, sharedInt);					
		this.serOut = out;
		this.serIn = in;
	...
	public int setCurrent(Piece piece, int x, int y) {
		...
		System.out.println("I am just before sending board information");
		try {
			serOut.writeObject(this.board);
			serOut.flush();
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		System.out.println("I am after sending board information");
	...
	public void addNewPiece() {
		...
		board.commit();
		currentPiece = null;
		String sendReq = "cccccc";

		try {
			serOut.writeObject(sendReq);
		...
		try {
			recString = (String) serIn.readObject();
		} catch (IOException e) {
			e.printStackTrace();
		...
		String[] recSArray = recString.substring(1, 17).split(", ");
		int pieceNum = Integer.parseInt(recSArray[0]);
		int[] rInt = new int[5];
		for (i = 0; i < 5; i++ ) {
			rInt[i] = Integer.parseInt(recSArray[i+1]);
		}		
		sharedQueue.updatePieces(rInt);
		Piece piece = pieces[pieceNum];
	...
	public void tick(int verb) {
		...
		if (failed && verb==DOWN && !moved) {	// it's landed
			...
			System.out.println("I am in the TICK before sending board information");
			try {
				serOut.writeObject(this.board);
			} catch (IOException e) {
				// TODO Auto-generated catch block
			...
			System.out.println("I am in the TICK after sending board information");
...
/*	public static JFrame createFrame(JTetris tetris) {
		...
		JTReceiver receiver = new JTReceiver(16, server);
		
		container.add(tetris, BorderLayout.WEST);
        container.add(receiver, BorderLayout.EAST);
		container.add(sharedQueue, BorderLayout.CENTER);
